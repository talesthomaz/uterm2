...
class TmuxControl(object):
    ...
    def remote_connect(self, command):
        if self.tmux:
            ...
            return
        # Implementing ssh port change
        self.remote_port = "22" if len(self.remote.split(":"))==1  else self.remote.split(":")[1]
        self.remote = self.remote.split(":")[0]
        popen_command = "ssh -p " + self.remote_port + " " + self.remote
        print(popen_command)
